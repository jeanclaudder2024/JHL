...
export const CompanyDashboard: React.FC<CompanyDashboardProps> = ({ user }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [employeesData, invoicesData] = await Promise.all([
        ...
        setEmployees(employeesData.map((emp: any) => ({
          ...
          name: emp.name,
          email: emp.email,
          status: emp.status === 'ACTIVE' ? 'Active' : 'Invited',
          dietary: emp.dietary || 'Standard',
        })));

        setInvoices(invoicesData.map((inv: any) => ({
          id: inv.id,
          date: new Date(inv.dueDate).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }),
          amount: `$${parseFloat(inv.amount).toLocaleString()}`,
          status: inv.status === 'PAID' ? 'Paid' : 'Pending',
          items: inv.items || 'Monthly Catering',
        })));
      } catch (error) {
