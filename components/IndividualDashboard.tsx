...
import { mealsApi, subscriptionsApi, mealAssignmentsApi } from '../services/api';
...
export const IndividualDashboard: React.FC<IndividualDashboardProps> = ({ user }) => {
  ...
  // Data States
  const [upcomingMeals, setUpcomingMeals] = useState<RatedMeal[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // Fetch upcoming meals for calendar
        const upcomingData = await mealAssignmentsApi.getUpcoming();

        // Fetch all assignments for history (client-side filtering for now as we don't have a specific history endpoint yet)
        // ideally backend should provide a history endpoint, for now we use getMy which returns scheduled. 
        // Wait, getMy returns SCHEDULED. We might need a history endpoint or check if we can get delivered ones.
        // Let's use getMy for now and see what it returns. The backend /my endpoint filters by status: 'SCHEDULED'.
        // We might need to update backend to get history (DELIVERED/CANCELLED or just past dates).
        // For now, let's just show upcoming in history too or maybe I should update backend to get all "my" assignments regardless of status?
        // Checking backend code: 
        // router.get('/my', ... where: { userId, status: 'SCHEDULED' } ...)
        // This is a limitation. I should probably update the backend to allow getting all assignments or add a history endpoint.
        // But for this step let's just hook up what we have.

        const transformAssignment = (a: any): RatedMeal => ({
          ...a.meal,
          id: a.id, // Override meal ID with assignment ID for list key
          realMealId: a.meal.id, // Keep reference to actual meal ID
          date: new Date(a.assignedDate).toISOString().split('T')[0],
          deliveryTime: a.deliveryTime,
          status: a.status
        });

        if (Array.isArray(upcomingData)) {
          setUpcomingMeals(upcomingData.map(transformAssignment));
        }

        // For history, we'll currently valid mock data until backend history is improved, 
        // OR we can fetch meals history using mealsApi.getHistory() which was already there/mocked?
        // mealsApi.getHistory calls /meals/history/me. Let's check that.
        // /meals/history/me returns MealRating with include meal.

        const historyData = await mealsApi.getHistory();
        if (Array.isArray(historyData)) {
          setHistoryMeals(historyData.map((item: any) => ({
            ...item.meal,
            id: item.id, // rating ID
            rating: item.rating,
            feedback: item.feedback,
            date: new Date(item.createdAt).toISOString().split('T')[0] // Rating date, roughly meal date
          })));
        }
